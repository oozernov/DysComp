...
exclude<-d_glm_fit %>%filter(slope_coef_t < 0 |slope_coef_t == 0 )
exclude2 = exclude %>% dplyr::group_by(PartID, group,age,word) %>%
  dplyr::summarise(m = mean(slope_coef_t))
table(exclude2$group,exclude2$word) #subjects to exclude per word
table(exclude2$group,exclude2$age) #subjects to exclude per word

d_glm_fit$slope_coef_t<-log(d_glm_fit$slope_coef_t)
hist(d_glm_fit$slope_coef_t)
...
model_slope<-lm(slope_coef_t~group*age*pair*word, data=d_glm_fit)
anova(model_slope)
eta_sq(model_slope)
...
lsmeans(model_slope, list(pairwise ~ pair), adjust = "tukey")
```
#### Try ANOVA-Type III
```{r, echo=FALSE}
# ##try Type III
# contrasts(d_glm_fit$age) <- contr.sum(2)
# contrasts(d_glm_fit$group) <- contr.sum(2)
# contrasts(d_glm_fit$word) <- contr.sum(6)
# contrasts(d_glm_fit$pair) <- contr.sum(2)
# 
# #
# require(car)
# model_slope3 <- lm(slope_coef_t ~ group*age*pair*word,, data = d_glm_fit)
# Anova(model_slope3, type = "III") #model is signular
# lsmeans(model_slope, list(pairwise ~ group), adjust = "tukey")
